...
```bash
sam-slack-new$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-slack-new$ sam local start-api
sam-slack-new$ curl http://localhost:3000/
```
...
```bash
sam-slack-new$ sam logs -n HelloWorldFunction --stack-name sam-slack-new --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-slack-new
```
